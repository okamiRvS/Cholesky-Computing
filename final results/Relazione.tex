...
Trovare più veloce, quello che occupa meno spazio

R ha import time maggiore sia su linux che su windows nonostante tempo di chol minore (verificare con grafico)

Grafico relazione tempo di chol e dimensione matrice cholesky ?

Dal punto di vista dell'accuratezza gli errori sono di ordini di grandezza molto vicini per tutti

Grafico occupazione memoria

C++ più lento su linux, in windows lenti R e C++ (notare che usiamo comunque metodi di calcolo del tempo diversi)
C++ gira su unico core

Facilità d'uso python schifo su windows per l'installazione ma codice ok, c++ scrittura del codice più complessa, matlab il più easy, R semplice
python poco documentato, il resto ok
pecca di c++ sono le prestazioni
